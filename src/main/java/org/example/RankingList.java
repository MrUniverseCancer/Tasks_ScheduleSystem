...
public class RankingList {
    ...
    public void addRankingList() {
        ...
        for (int i = 0; i < listsItems.size(); i++) {
            if (listsItems.get(i).getId() == 0 || listsItems.get(i).getId() == 1) {
                list_id_work.add(listsItems.get(i).getId());
            }
        }
        for (int i = 0; i < dataItems.size(); i++) {
            if (!list_id_work.contains(dataItems.get(i).getList_id())) {
                dataItems.remove(i);
                i--;
            }
        }
    ...
    public void rankingALG(ArrayList<DataItem> dataItems) {
